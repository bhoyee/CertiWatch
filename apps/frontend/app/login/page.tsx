...
export default function LoginPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL ?? "http://localhost:5002"}/api/auth/magic-link`, {
        method: "POST",
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4">
      <div className="max-w-md w-full rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
        ...
        <p className="mt-2 text-sm text-slate-600">Enter your email and we’ll send you a sign-in link.</p>
        ...
        {status === "sent" && <p className="mt-4 text-sm text-green-600">Sent! Check your email.</p>}
