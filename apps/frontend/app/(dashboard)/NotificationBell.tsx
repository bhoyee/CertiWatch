  ...
  return (
    <div className="relative" ref={containerRef}>
      <button
        ...
        className="relative flex h-10 w-10 items-center justify-center rounded-full border border-rose-200 bg-rose-50 text-rose-600 shadow-sm transition hover:bg-rose-100"
      >
      ...
      {open && (
        <div className="absolute right-0 z-50 mt-2 w-80 rounded-2xl border border-slate-200 bg-white shadow-lg sm:w-96">
            ...
            {loadingFeed ? (
              ...
              items.map((n) => (
                <Link
                  key={n.id}
                  href={
                    isSupportType(n.type) && n.ticketId
                      ? `/support?ticket=${n.ticketId}`
                ...
                >
                  <span
                    className={`mt-1.5 h-2 w-2 flex-shrink-0 rounded-full ${
                      n.type === "expired" ? "bg-rose-500" : isSupportType(n.type) ? "bg-indigo-500" : "bg-amber-500"
                    } ${n.isRead ? "opacity-0" : ""}`}
                  />
          ...
          <Link
            ...
            className="block border-t border-slate-100 px-4 py-2.5 text-center text-sm font-medium text-indigo-600 hover:bg-slate-50 hover:text-indigo-700"
          >
